...
st.set_page_config(
    page_title="Gerador de Relatórios HPTECH", page_icon="hptechICO.png", layout="wide"
)
...
class PDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(
            0, 15, f"RELATÓRIO DE ATENDIMENTO Nº {ra_mostrar}", ln=True, align="L"
        )


def enviar_relatorio_email(
    arquivos_anexos, servidor_smtp, porta, email_remetente, senate, destinatario
):
    ...
    nome_base = (
        os.path.basename(primeiro_arquivo)
        .replace(".pdf", "")
        .replace(".xlsx", "")
        .strip()
    )
...
Hudson Valente"""
    ...
    arquivos_para_enviar = (
        arquivos_anexos if isinstance(arquivos_anexos, list) else [arquivos_anexos]
    )
...
email_destinatario = st.sidebar.text_input(
    ...
    value="financeiro@crti.com.br"
)
senha_app = st.sidebar.text_input(
    "Senha App Gmail:",
    value="fzau tvih zlsn xadi",
    type="password"
)
...
if btn_gerar:
    ...
    with st.spinner("🛠️ Gerando arquivos PDF e Excel idênticos..."):
        ...
        # --- Ajuste de colunas obrigatórias ---
        cols_obr = [
            "CLIENTE", "OBSERVAÇÕES", "CONSULTOR", "SOLICITANTE", "PARTICIPANTE",
            "FORMA", "RA", "LOCAL", "SITUACAO_RA", "HR_INICIO_D", "HR_FIM_D",
            "TOTAL_HR_D", "KM_D", "FORMA_D", "DESCRICAO_D", "HR_INICIO", "HR_FIM",
            "TOTAL_HR", "DATA", "DESCRICAO_P", "RESPONSAVEL_P", "STATUS_P"
        ]
...
data_inicio_rel = (
    datas_grupo.min().strftime("%d/%m/%Y")
    if pd.notna(datas_grupo.min())
    else "01/01/2026"
)
data_fim_rel = (
    datas_grupo.max().strftime("%d/%m/%Y")
    if pd.notna(datas_grupo.max())
    else "31/12/2026"
)
dt_obj = (
    datas_grupo.max().to_pydatetime()
    if pd.notna(datas_grupo.max())
    else datetime.now()
)
...
for val in grupo["KM_D"]:
    ...
    if v not in ["", "nan", "None", "0", "0.0"]:
        try:
            kv = float(v)
            total_km += kv
            tem_desl = True if kv > 0 else False
        except:
            pass
...
ra_str = (
    str(int(float(texto_ra)))
    if texto_ra != "" and texto_ra.replace(".", "", 1).isdigit()
    else "S/N"
)
...
for _, linha in grupo.iterrows():
    ...
    da = (
        pd.to_datetime(linha["DATA"]).strftime("%d/%m/%Y")
        if pd.notnull(linha["DATA"])
        else ""
    )
    ...
    pdf.set_font("Arial", "B", 10)
    pdf.cell(22, 5, "Hora Final: ", ln=False)
    pdf.set_font("Arial", "", 10)
    ...
    pdf.rect(x_i, y_i, 190, pdf.get_y() - y_i)
            if tem_desl:
                ...
                pdf.ln(2); pdf.set_font("Arial", "B", 10); pdf.set_fill_color(0, 112, 192); pdf.set_text_color(255, 255, 255); pdf.cell(190, 10, "DESLOCAMENTOS", border=1, ln=True, fill=True, align="C"); pdf.set_text_color(0, 0, 0)
                for _, linha_d in grupo.iterrows():
                    km_s = str(linha_d.get("KM_D", "")).strip().replace(',', '.'); sub_hi_d = str(linha_d["HR_INICIO_D"])[0:5] if pd.notnull(linha_d["HR_INICIO_D"]) else "00:00"; sub_hf_d = str(linha_d["HR_FIM_D"])[0:5] if pd.notnull(linha_d["HR_FIM_D"]) else "00:00"; sub_tt_d = str(linha_d["TOTAL_HR_D"])[0:5] if pd.notnull(linha_d["TOTAL_HR_D"]) else "00:00"
                    if km_s in ["", "nan", "None", "0", "0.0"]: continue
                    if pdf.get_y() > 245: pdf.add_page()
                    y_i = pdf.get_y(); x_i = 10; sub_dd = pd.to_datetime(linha_d["DATA"]).strftime("%d/%m/%Y") if pd.notnull(linha_d["DATA"]) else ""; sub_ds_d = str(linha_d.get("DESCRICAO_D", "")).strip(); sub_fm_d = str(linha_d.get("FORMA_D", "Carro Próprio")).strip(); pdf.set_xy(x_i + 2, y_i + 2); pdf.set_font('Arial', 'B', 10); pdf.cell(22, 5, "Data: ", ln=False); pdf.set_font('Arial', '', 10); pdf.cell(30, 5, sub_dd, ln=False); pdf.set_font('Arial', 'B', 10); pdf.cell(22, 5, "Hora Início: ", ln=False); pdf.set_font('Arial', '', 10); pdf.cell(15, 5, sub_hi_d, ln=False); pdf.set_font('Arial', 'B', 10); pdf.cell(20, 5, "Hora Final: ", ln=False); pdf.set_font('Arial', '', 10); pdf.cell(38, 5, sub_hf_d, ln=False); pdf.set_font('Arial', 'B', 10); pdf.cell(30, 5, "Total: ", ln=False); pdf.set_font('Arial', '', 10); pdf.cell(0, 5, sub_tt_d, ln=True); pdf.set_x(x_i + 2); pdf.set_font('Arial', 'B', 10); pdf.cell(22, 5, "Distância: ", ln=False); pdf.set_font('Arial', '', 10); pdf.cell(30, 5, f"{km_s} km", ln=False); pdf.set_font('Arial', 'B', 10); pdf.cell(45, 5, "Forma de Deslocamento: ", ln=False); pdf.set_font('Arial', '', 10); pdf.cell(30, 5, sub_fm_d, ln=False); pdf.set_font('Arial', 'B', 10); pdf.cell(20, 5, "Consultor: ", ln=False); pdf.set_font('Arial', '', 10); pdf.cell(0, 5, consultor, ln=True); pdf.set_x(x_i + 2); pdf.set_font('Arial', 'B', 10); pdf.cell(22, 5, "Descrição: ", ln=False); pdf.set_font('Arial', '', 10); pdf.multi_cell(0, 5, sub_ds_d if sub_ds_d else "-"); pdf.set_y(pdf.get_y() + 2); pdf.rect(x_i, y_i, 190, pdf.get_y() - y_i)
            if tem_pendencias:
                if pdf.get_y() > 220: pdf.add_page()
                pdf.ln(2); pdf.set_font("Arial", "B", 10); pdf.set_fill_color(255, 192, 0); pdf.set_text_color(0, 0, 0); pdf.cell(190, 10, "PENDÊNCIAS", border=1, ln=True, fill=True, align="C")
                for _, linha_p in grupo_pendencias.iterrows():
                    if pdf.get_y() > 245: pdf.add_page()
                    y_i = pdf.get_y(); x_i = 10; dp_data = pd.to_datetime(linha_p["DATA"]).strftime("%d/%m/%Y") if pd.notnull(linha_p["DATA"]) else data_inicio_rel; desc_p = str(linha_p["DESCRICAO_P"]).strip(); resp_p = str(linha_p["RESPONSAVEL_P"]).strip(); status_p = str(linha_p["STATUS_P"]).strip(); pdf.set_xy(x_i + 2, y_i + 2); pdf.set_font('Arial', 'B', 10); pdf.cell(14, 5, "Data: ", ln=False); pdf.set_font('Arial', '', 10); pdf.cell(30, 5, dp_data, ln=False); pdf.set_font('Arial', 'B', 10); pdf.cell(25, 5, "Responsável: ", ln=False); pdf.set_font('Arial', '', 10); pdf.cell(50, 5, resp_p, ln=False); pdf.set_font('Arial', 'B', 10); pdf.cell(15, 5, "Status: ", ln=False); pdf.set_font('Arial', '', 10); pdf.cell(0, 5, status_p, ln=True); pdf.set_x(x_i + 2); pdf.set_font('Arial', 'B', 10); pdf.cell(22, 5, "Descrição: ", ln=False); pdf.set_font('Arial', '', 10); pdf.multi_cell(164, 5, desc_p); pdf.set_y(pdf.get_y() + 2); pdf.rect(x_i, y_i, 190, pdf.get_y() - y_i)
            if pdf.get_y() > 220: pdf.add_page()
            pdf.ln(5); pdf.set_font("Arial", "", 10); pdf.set_text_color(0, 0, 0); pdf.cell(0, 8, f"Curitiba, {data_rodape}.", ln=True); pdf.ln(4); pdf.set_font("Arial", "", 10); pdf.set_text_color(255, 0, 0); pdf.multi_cell(0, 5, "As horas referentes aos atendimentos e despesas de viagens serão faturadas conforme acerto prévio. Declaro que os serviços descritos neste relatório foram realizados conforme solicitado e estão em conformidade.", align="C"); pdf.ln(10); pdf.set_text_color(0, 0, 0); pdf.cell(90, 8, "__________________________________", align="C"); pdf.cell(10); pdf.cell(90, 8, "__________________________________", align="C", ln=True); pdf.cell(90, 2, consultor, align="C"); pdf.cell(7); pdf.cell(90, 2, solicitante, align="C", ln=True); pdf.set_font("Arial", "", 8); pdf.cell(90, 7, "CRTI", align="C"); pdf.cell(7); pdf.cell(90, 7, cliente, align="C", ln=True); pdf.cell(90, 2, f"RELATÓRIO DE ATENDIMENTO Nº {ra_str}", align="C"); pdf.cell(7); pdf.cell(90, 2, f"RELATÓRIO DE ATENDIMENTO Nº {ra_str}", ln=True, align="C"); pdf.output(file_pdf); arquivos_saida.append(file_pdf)
            wb = xlsxwriter.Workbook(file_xlsx); ws = wb.add_worksheet("Relatório"); ws.set_paper(9); ws.fit_to_pages(1, 0); ws.set_margins(left=0.2, right=0.2, top=0.4, bottom=0.4); ws.set_column('A:A', 13); ws.set_column('B:B', 15); ws.set_column('C:C', 13); ws.set_column('D:D', 11); ws.set_column('E:E', 13); ws.set_column('F:F', 13); ws.set_column('G:G', 8); ws.set_column('H:H', 11); f_title = wb.add_format({'bold': True, 'font_size': 13}); f_hdr_u = wb.add_format({'bold': True, 'underline': True, 'font_size': 10}); f_bold = wb.add_format({'bold': True, 'font_size': 10, 'valign': 'vcenter'}); f_norm = wb.add_format({'font_size': 10, 'valign': 'vcenter'}); f_blue = wb.add_format({'bold': True, 'bg_color': '#0070c0', 'font_color': 'white', 'align': 'center', 'valign': 'vcenter', 'border': 1}); f_yellow = wb.add_format({'bold': True, 'bg_color': '#FFC000', 'font_color': 'black', 'align': 'center', 'valign': 'vcenter', 'border': 1}); f_red = wb.add_format({'font_color': 'red', 'align': 'center', 'valign': 'vcenter', 'text_wrap': True}); f_center = wb.add_format({'align': 'center', 'valign': 'top', 'font_size': 10}); f_sign = wb.add_format({'top': 1, 'align': 'center', 'font_size': 10, 'valign': 'top'}); f_TL = wb.add_format({'top': 1, 'left': 1, 'bold': True, 'valign': 'vcenter'}); f_T = wb.add_format({'top': 1, 'valign': 'vcenter'}); f_T_b= wb.add_format({'top': 1, 'bold': True, 'valign': 'vcenter'}); f_TR = wb.add_format({'top': 1, 'right': 1, 'valign': 'vcenter'}); f_ML = wb.add_format({'left': 1, 'bold': True, 'valign': 'top'}); f_M = wb.add_format({'valign': 'top'}); f_M_b= wb.add_format({'bold': True, 'valign': 'top'}); f_merge_mid = wb.add_format({'valign': 'top', 'text_wrap': True, 'right': 1}); f_merge_bot = wb.add_format({'bottom': 1, 'valign': 'top', 'text_wrap': True, 'right': 1}); f_BL = wb.add_format({'bottom': 1, 'left': 1, 'bold': True, 'valign': 'top'})
            if os.path.exists(LOGO): ws.insert_image('F1', LOGO, {'x_offset': 10, 'y_offset': 2, 'x_scale': 2.00, 'y_scale': 2.00})
            ws.write('A2', f"RELATÓRIO DE ATENDIMENTO Nº {ra_str}", f_title); ws.write('A4', "DADOS GERAIS:", f_hdr_u); ws.write('F4', "RESUMO DO RELATÓRIO:", f_hdr_u); ws.write('A5', "Cliente:", f_bold); ws.merge_range('B5:D5', cliente[:45], f_norm); ws.write('F5', "Período:", f_bold); ws.merge_range('G5:H5', f"{data_inicio_rel} até {data_fim_rel}", f_norm); ws.write('A6', "Solicitante:", f_bold); ws.merge_range('B6:D6', solicitante, f_norm); ws.write('F6', "Total de Horas:", f_bold); ws.merge_range('G6:H6', total_hr_str, f_norm); ws.write('A7', "Tipo de Atend.:", f_bold); ws.merge_range('B7:D7', "Implantação CRTI", f_norm); ws.write('F7', "Deslocamento:", f_bold); ws.merge_range('G7:H7', total_hr_str_d, f_norm); ws.write('A8', "Unidade:", f_bold); ws.merge_range('B8:D8', local, f_norm); ws.write('F8', "Distância (KM):", f_bold); ws.merge_range('G8:H8', f"{total_km:.2f} km".replace('.', ','), f_norm); row = 10; ws.merge_range(f'A{row}:H{row}', "DESCRIÇÃO DAS ATIVIDADES", f_blue); ws.set_row(row - 1, 18); row += 1
            for _, linha_a in grupo.iterrows():
                d = pd.to_datetime(linha_a["DATA"]).strftime("%d/%m/%Y") if pd.notnull(linha_a["DATA"]) else ""; ob = str(linha_a["OBSERVAÇÕES"]).strip(); fo = str(linha_a.get("FORMA", "Remoto")).strip(); pa = str(linha_a["PARTICIPANTE"]).strip() or participante_padrao; hi = str(linha_a["HR_INICIO"])[0:5] if pd.notnull(linha_a["HR_INICIO"]) else "00:00"; hf = str(linha_a["HR_FIM"])[0:5] if pd.notnull(linha_a["HR_FIM"]) else "00:00"; tt = str(linha_a["TOTAL_HR"])[0:5] if pd.notnull(linha_a["TOTAL_HR"]) else "00:00"; ws.write(f'A{row}', "Data:", f_TL); ws.write(f'B{row}', d, f_T); ws.write(f'C{row}', "Hora Início:", f_T_b); ws.write(f'D{row}', hi, f_T); ws.write(f'E{row}', "Hora Final:", f_T_b); ws.write(f'F{row}', hf, f_T); ws.write(f'G{row}', "Total:", f_T_b); ws.write(f'H{row}', tt, f_TR); row += 1; ws.write(f'A{row}', "Consultor:", f_ML); ws.merge_range(f'B{row}:D{row}', consultor, f_M); ws.merge_range(f'E{row}:F{row}', "Forma de Atendimento:", f_M_b); ws.merge_range(f'G{row}:H{row}', fo, f_merge_mid); row += 1; linhas_obs = max(1, len(ob) // 90 + 1); ws.write(f'A{row}', "Atividade:", f_ML); ws.merge_range(f'B{row}:H{row}', ob if ob else "-", f_merge_mid); ws.set_row(row - 1, 15 * linhas_obs); row += 1; linhas_pa = max(1, len(pa) // 90 + 1); ws.write(f'A{row}', "Participante:", f_BL); ws.merge_range(f'B{row}:H{row}', pa if pa else "-", f_merge_bot); ws.set_row(row - 1, 15 * linhas_pa); row += 1
            if tem_desl:
...
for _, linha_p in grupo_pendencias.iterrows():
    ...
    ws.write(
        f'B{row}',
        pd.to_datetime(linha_p["DATA"]).strftime("%d/%m/%Y")
        if pd.notnull(linha_p["DATA"])
        else data_inicio_rel,
        f_T
    )
...
ws.merge_range(
    f'A{row}:H{row}',
    "As horas referentes aos atendimentos e despesas de viagens serão "
    "faturadas conforme acerto prévio. Declaro que os serviços descritos "
    "neste relatório foram realizados conforme solicitado."
)
ws.set_row(row - 1, 30)
row += 5
...
resumo_status.success(
    f"✅ Feito! Foram gerados {len(arquivos_saida)} arquivos da aba '{aba_selecionada}'."
)
    ...
    for i, path in enumerate(arquivos_saida):
        ...
        with open(path, "rb") as file:
            cols_dw[i % 3].download_button(
                label=f"Baixar {file_ext}: {nome_arq[:15]}...",
                data=file,
                file_name=nome_arq,
                key=path
            )
...
if st.session_state.relatorios_gerados:
    ...
    arquivos_exibicao = [
        f for f in arquivos_pasta if f.endswith(".pdf") or f.endswith(".xlsx")
    ]
...
def confirmar_envio_atendimentos_popup(arquivos_validos):
    ...
    with col_p1:
        if st.button("✅ Sim, Disparar Todos", use_container_width=True):
            import time
            st.subheader("🚀 Iniciando disparo Mercedes...")
            sucessos = 0
            barra = st.progress(0)
