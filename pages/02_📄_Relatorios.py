...
# CARREGAMENTO COPIADO EXATAMENTE DA PÁGINA 02
        ...
        with st.spinner(f"⏳ Processando atendimentos da aba '{aba_mes_selecionada}'..."):
            try:
                ...
                if atendimentos_cliente.empty:
                    ...
                    # LÓGICA DA PÁGINA 02: Transforma as colunas em dicionários puros de valores para quebrar o congelamento da memória
                    dict_part = atendimentos_cliente["PARTICIPANTE"].to_dict() if "PARTICIPANTE" in atendimentos_cliente.columns else atendimentos_cliente["PARTICIPANTES"].to_dict()
                    dict_desc = atendimentos_cliente["DESC_PRES"].to_dict()
                    dict_obs = atendimentos_cliente["OBS_PRES"].to_dict()
                    dict_entrada = atendimentos_cliente["ENTRADA"].to_dict()
                    dict_saida = atendimentos_cliente["SAÍDA"].to_dict() if "SAÍDA" in atendimentos_cliente.columns else atendimentos_cliente["SAIDA"].to_dict()
                    dict_modulo = atendimentos_cliente["MÓDULO / ATIVIDADE"].to_dict() if "MÓDULO / ATIVIDADE" in atendimentos_cliente.columns else atendimentos_cliente["MODULO / ATIVIDADE"].to_dict()
                    dict_data_dia = atendimentos_cliente[col_data].to_dict()

                    # O laço varre os índices reais mapeando os valores diretos das chaves do dicionário
                    for idx in atendimentos_cliente.index:
                        ...
                        # Captura as horas e limpa os segundos de forma isolada por registro
