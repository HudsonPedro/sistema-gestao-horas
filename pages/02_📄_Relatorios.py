...
class PDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(
            0,
            15,
            f"RELATÓRIO DE ATENDIMENTO Nº {ra_mostrar}",
            ln=True,
            align="L",
        )
...
data_inicio_selecionada = st.sidebar.date_input(
    "**Data Início**", value=pd.to_datetime(min_data_aba).date()
)
data_fim_selecionada = st.sidebar.date_input(
    "**Data Fim**", value=pd.to_datetime(max_data_aba).date()
)
...
"Enviar para (Destinatário):", value="financeiro@crti.com.br"
)
senha_app = st.sidebar.text_input(
"Senha App Gmail:", value="fzau tvih zlsn xadi", type="password"
)
btn_enviar_emails = st.sidebar.button(
"📧 ENVIAR POR E-MAIL", type="primary", use_container_width=True
)
st.sidebar.markdown("---")
st.sidebar.header("--- Gerenciamento de Arquivos ---")
if st.sidebar.button(
"🗑️ Limpar Relatórios Antigos",
type="secondary",
use_container_width=True,
):
arquivos_para_remover = glob.glob(
os.path.join(PASTA_SAIDA, ".pdf")
) + glob.glob(os.path.join(PASTA_SAIDA, ".xlsx"))
...
datas_grupo = pd.to_datetime(
grupo["DATA"], errors="coerce", dayfirst=True
)
data_inicio_rel = (
...
pdf.cell(
0, 5, f"{data_inicio_rel} até {data_fim_rel}", ln=True
)
pdf.set_x(10)
pdf.set_font("Arial", "B", 10)
...
pdf.set_font("Arial", "", 10)
pdf.cell(
66.5, 5, "Implantação do Sistema CRTI ERP", ln=False
)
pdf.set_x(114.5)
pdf.set_font("Arial", "B", 10)
...
hi = (
str(linha["HR_INICIO"])[0:5]
if pd.notnull(linha["HR_INICIO"])
else "00:00"
)
hf = (
str(linha["HR_FIM"])[0:5]
if pd.notnull(linha["HR_FIM"])
else "00:00"
)
tt = (
str(linha["TOTAL_HR"])[0:5]
if pd.notnull(linha["TOTAL_HR"])
else "00:00"
)
...
0,
5,
str(linha.get("FORMA", "Remoto")).strip(),
ln=True,
)
pdf.set_x(x_i + 2)
pdf.set_font("Arial", "B", 10)
...
0,
5,
str(linha["PARTICIPANTE"]).strip()
or participante_padrao,
)
...
hi_d = (
str(linha["HR_INICIO_D"])[0:5]
if pd.notnull(linha["HR_INICIO_D"])
else "00:00"
)
hf_d = (
str(linha["HR_FIM_D"])[0:5]
if pd.notnull(linha["HR_FIM_D"])
else "00:00"
)
tt_d = (
str(linha["TOTAL_HR_D"])[0:5]
if pd.notnull(linha["TOTAL_HR_D"])
else "00:00"
)
...
pdf.cell(22, 5, "Data: ", ln=False)
pdf.set_font("Arial", "", 10)
pdf.cell(30, 5, dd, ln=False)
pdf.set_font("Arial", "B", 10)
pdf.cell(22, 5, "Hora Início: ", ln=False)
pdf.set_font("Arial", "", 10)
pdf.cell(15, 5, hi_d, ln=False)
pdf.set_font("Arial", "B", 10)
pdf.cell(22, 5, "Hora Final: ", ln=False)
pdf.set_font("Arial", "", 10)
pdf.cell(38, 5, hf_d, ln=False)
pdf.set_font("Arial", "B", 10)
pdf.cell(30, 5, "Total: ", ln=False)
pdf.set_font("Arial", "", 10)
pdf.cell(0, 5, tt_d, ln=True)
pdf.set_x(x_i + 2)
pdf.set_font("Arial", "B", 10)
...
pdf.set_font("Arial", "", 10)
pdf.multi_cell(0, 5, ds_d if ds_d else "-")
pdf.set_y(pdf.get_y() + 2)
pdf.rect(x_i, y_i, 190, pdf.get_y() - y_i)
if pdf.get_y() > 220:
pdf.add_page()
...
0,
5,
"As horas referentes aos atendimentos e despesas de viagens serão faturadas conforme acerto prévio. Declaro que os serviços descritos neste relatório foram realizados conforme solicitado e estão em conformidade.",
align="C",
)
pdf.ln(10)
pdf.set_text_color(0, 0, 0)
pdf.cell(90, 8, "", align="C")
pdf.cell(10)
pdf.cell(
90, 8, "", align="C", ln=True
)
...
pdf.cell(
90, 2, f"RELATÓRIO DE ATENDIMENTO Nº {ra_str}", align="C"
)
pdf.cell(7)
pdf.cell(
90,
2,
f"RELATÓRIO DE ATENDIMENTO Nº {ra_str}",
ln=True,
align="C",
)
pdf.output(file_pdf)
...
f_center = wb.add_format(
{"align": "center", "valign": "top", "font_size": 10}
)
f_sign = wb.add_format(
{"top": 1, "align": "center", "font_size": 10, "valign": "top"}
)
f_TL = wb.add_format(
{"top": 1, "left": 1, "bold": True, "valign": "vcenter"}
)
...
f_merge_bot = wb.add_format(
{"bottom": 1, "valign": "top", "text_wrap": True, "right": 1}
)
f_BL = wb.add_format(
{"bottom": 1, "left": 1, "bold": True, "valign": "top"}
)
if os.path.exists(LOGO):
ws.insert_image(
"F1",
...
ws.merge_range(
"G5:H5", f"{data_inicio_rel} até {data_fim_rel}", f_norm
)
...
ws.merge_range(
"G8:H8",
f"{total_km:.2f} km".replace(".", ","),
f_norm,
)
row = 10
ws.merge_range(
f"A{row}:H{row}", "DESCRIÇÃO DAS ATIVIDADES", f_blue
)
ws.set_row(row - 1, 18)
row += 1
for _, linha in grupo.iterrows():
...
st.write(
"Você tem certeza que deseja disparar os relatórios de atendimento em lote?"
)
